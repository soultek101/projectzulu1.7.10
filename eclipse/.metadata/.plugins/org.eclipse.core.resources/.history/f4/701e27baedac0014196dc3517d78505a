...
public class ModelCamel extends ModelBase
{
  ...
  public void setLivingAnimations(EntityLivingBase par1EntityLiving, float par2, float par3, float par4) {
	...
	hip3.rotateAngleX = (float) (MathHelper.cos(par2 * 0.6662F) * 1.8F * ModelHelper.abs(Math.log(par3 + 1)));
				
	hip4.rotateAngleX = (float) (MathHelper.cos(par2 * 0.6662F + (float) Math.PI) * 1.8F * ModelHelper
			.abs(Math.log(par3 + 1)));
  ...
  public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5, Entity entity)
  {
    ...
    HEAD.rotateAngleX = f4 / (180F / (float)Math.PI); //Math.min(Math.max(f4, -14), +35) * (float) (Math.PI / 180f);
